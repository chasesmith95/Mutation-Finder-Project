...
class ExcelInput(ttk.Frame):
    def __init__(self, master=None):
        ...
        self.amerLengthList=["octamers (8 aa)","nonamers (9 aa)","decamers (10 aa)",  "endecamers (11 aa)", "15 - mers (15 aa) for MHC Type II only", "All mers"]
        self.amerLengthDict= {"octamers (8 aa)": 8, "nonamers (9 aa)": 9 ,"decamers (10 aa)": 10,  "endecamers (11 aa)": 11, "15 - mers (15 aa) for MHC Type II only": 15, "All mers": 0}
        ...
        self.listFileLabels= ["Input Folder Path", "Input File", "Input Sheet", "Output File", "Output Sheet", "Peptide Length", "Immune Type", "Returns per Mutation"]
        ...
        while i<len(fileList):
            ...
            if((str(files).find(' SY.txt'))>=0):
                ...
                try:
                    print(files)
                    beginInt=files.find('.')
                    fileEndVal=files[beginInt:]
                    filesOut= files.replace(fileEndVal, ' SY.txt')
                    print(filesOut)
                    fileIn =files
                    self.excel=Excel(fileIn, str(self.input_sheetStr) ,filesOut,'', (self.input_listStr), str(self.amerLengthDict[self.amerLength.get()]), str(self.geneType_col.get()),str(self.numSYReturns.get()), bool(self.ensembl_var.get()))
                    self.excel.runExcel()
                except:
